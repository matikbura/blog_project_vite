<template>
  <div v-loading="isLoad" >
    <div class="divide-y divide-gray-200  px-3 overflow-y-auto"  style="height: 800px" v-infinite-scroll="load">
      <div v-for="(item) in data.rows " :key="item.articleId"
           class=" flex p-4 cursor-pointer duration-300 hover:bg-gray-100 group">
        <div class="w-full">
            ...
            <div class="flex space-x-6">
              <div class="text-gray-500">
                <el-tooltip content="预览" placement="top" effect="light">
                  <button
                      class="group-hover:opacity-100 duration-300 opacity-0 text-lg rounded-full p-1 hover:bg-red-500 hover:text-white"
                      @click="preview(item)">
                    <span class="iconify" data-icon="ic:round-preview"></span>
                  </button>
                </el-tooltip>
                <el-tooltip content="放入回收站" placement="top" effect="light">
                  <button
                      class="group-hover:opacity-100 duration-300 opacity-0 text-lg rounded-full p-1 hover:bg-red-500 hover:text-white">
                    <span class="iconify" data-icon="ic:outline-delete"></span>
                  </button>
                </el-tooltip>


              </div>
              <span class="text-xs text-green-500" v-if="item.status===0">草稿箱</span>
              <span class="text-xs text-green-500" v-if="item.status===1">审核中 </span>
              <span class="text-xs text-blue-500" v-else-if="item.status===2">审核通过</span>
              <span class="text-xs text-red-500" v-else-if="item.status===3">审核不通过</span>
              <span class="text-xs text-gray-500" v-else-if="item.status===4">已发布</span>
              <span class="text-xs text-purple-500" v-else-if="item.status===5">回收站</span>
            </div>
          </div>
...
const data = ref<Pages<Article>>({
  page: 1,queryParams: {uid:$user.user.uid} as Article,rows: [],  size: 10,
})
onMounted(() => {
  //初始化 获取所有 文章属于自己的
  isLoad.value = true
