          ...
          <div class="flex space-x-6">
            <div  class="text-gray-500">
              ...
              <el-tooltip content="发布" placement="top" effect="light">
                <button  class="group-hover:opacity-100 duration-300 opacity-0 text-lg rounded-full p-1 hover:bg-red-500 hover:text-white" @click="publish(item)">
                  <span class="iconify" data-icon="ic:baseline-publish"></span>
                </button>
              </el-tooltip>
            </div>
          </div>
...
const publish = (article:Article)=>{
  articleApi.updateArticle({articleId:article.articleId,status:4}).then(()=>{
    data.value.rows = data.value.rows!.filter(item=>item.articleId!==article.articleId)
...
const data = ref<Pages<Article>>({
  page: 1,queryParams: {uid:$user.user.uid,status:2} as Article,rows: [],  size: 10,
})
onMounted(()=>{
  //初始化 获取所有 文章属于自己的
  isLoad.value=true
